...
subroutine Runge_Kutta_2s(Time_Domain,Differential_operator,nbodys,Solution)

real, intent(in) :: Time_Domain(0:)
real, intent(inout) ::  Solution(0:,0:)
real :: inct, k1(2), k2(2)
integer, intent (in) :: nbodys
integer :: i
...
do i=0,(size(Time_Domain)-2)
 inct=Time_Domain(i+1)-Time_Domain(i)
   k1=Differential_Operator(Solution(i,:),Time_Domain(i),nbodys)
   k2=Differential_Operator(Solution(i,:)+inct*k1,Time_Domain(i)+inct,nbodys)
